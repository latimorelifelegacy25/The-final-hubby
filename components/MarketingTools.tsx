...
import { chatWithGemini, generateBulkCampaign } from "../services/geminiService";
...
const CanvaSpecGenerator = () => {
  ...
  const generate = async () => {
    if (!goal.trim()) return;
    setLoading(true);
    const res = await chatWithGemini(`Generate a Canva Creative Spec (copy/paste ready) for: "${goal}". Focus on visual hierarchy and high-impact educational copy for Central PA families.`, []);
       ...
       <div className="space-y-6">
          ...
          <div className="space-y-2">
             ...
             <textarea 
               value={goal}
               onChange={(e) => setGoal(e.target.value)}
               placeholder="e.g., 'A Facebook post to announce 10-minute term life protection for Schuylkill county parents'"
